...
func main() {
	...
	// Seed demo data only on first boot (when data/users.json is empty or absent)
	users, _ := userService.GetAllUsers()
	if len(users) == 0 {
		seedDemoData(memoryStore, userService, libraryService)
		fmt.Println("🌱 First boot: seeded demo users")
	} else {
		fmt.Printf("📂 Loaded %d users from data/users.json\n", len(users))
	}
	...
	c := cors.New(cors.Options{
		AllowedOrigins: []string{
			...
			"http://localhost:5173",
			"http://localhost:8080",
			"https://frontend-one-lake-66.vercel.app",
			"https://*.vercel.app",
		},
