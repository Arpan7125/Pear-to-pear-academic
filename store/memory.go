...
// parseTime safely parses an RFC3339 timestamp string.
...
// NewMemoryStore creates a new in-memory store and loads persisted users from JSON
func NewMemoryStore() *MemoryStore {
	...
	// Load previously saved users (returns 0 on first boot)
	m.loadUsers()
	return m
}
...
// Create adds a new user and persists to JSON
func (m *MemoryStore) Create(user *models.User) error {
	...
	m.saveUsers() // persist to data/users.json
	return nil
}
...
// UpdateUser modifies user data and persists to JSON
...
// DeleteUser removes a user and persists to JSON
