...
export async function downloadResource(id: string, userId: string) {
    // First register the download (increment count) via a lightweight fetch
    try {
        await fetch(`${BASE_URL}/resources/${id}/download?user_id=${userId}&register=true`, { method: 'GET' });
    } catch {
        // ignore registration errors — still proceed with download
    }
    // Open the actual file download URL directly so the browser handles the binary
    const downloadUrl = `${BASE_URL}/resources/${id}/download?user_id=${userId}`;
