...
func (s *UserService) CreateUser(username, email, password string) (*models.User, error) {
	...
	if err != nil {
		return nil, errors.NewOperationError("CreateUser", "failed to hash password", err)
	}

	// Create user with constructor
	user := models.NewUser(id, username, email)
	user.Password = string(hashed) // Store the bcrypt hash, never plain text
...
// AuthenticateUser finds a user by username and verifies the password with bcrypt
func (s *UserService) AuthenticateUser(username, password string) (*models.User, error) {
	...
	for _, user := range users {
		if user.Username == username {
			// CompareHashAndPassword returns nil on a match
