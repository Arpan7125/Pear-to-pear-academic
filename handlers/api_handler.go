...
// Uses bcrypt-backed AuthenticateUser to verify credentials securely
...
func (h *APIHandler) DownloadResource(w http.ResponseWriter, r *http.Request) {
	...
	// If the file doesn't exist (e.g. seeded demo data), generate a placeholder
	if _, err := os.Stat(filePath); os.IsNotExist(err) {
		...
		placeholder := fmt.Sprintf(
			"=== P2P Academic Library — Demo Resource ===\n\nTitle:    %s\nSubject:  %s\nFilename: %s\nID:       %s\n\nThis is a placeholder file generated for demonstration purposes.\nIn a production deployment, the actual academic document would be stored here.\n",
			resource.Title, resource.Subject, resource.Filename, resource.ID,
		)
		if writeErr := os.WriteFile(filePath, []byte(placeholder), 0644); writeErr != nil {
			writeError(w, http.StatusInternalServerError, "Could not generate placeholder file")
			return
		}
	}
